...
before_state = contract.functions.electionState().call()
candidates = contract.functions.getCandidatesCount().call()

print("BEFORE STATE:", before_state)
print("CANDIDATES:", candidates)

assert before_state == 0, "Election already started"
assert candidates > 0, "No candidates added"

nonce = w3.eth.get_transaction_count(ADMIN)

tx = contract.functions.startElection().build_transaction({
    "from": ADMIN,
    "nonce": nonce,
    "gas": 300_000,
...
print("AFTER STATE:", contract.functions.electionState().call())
